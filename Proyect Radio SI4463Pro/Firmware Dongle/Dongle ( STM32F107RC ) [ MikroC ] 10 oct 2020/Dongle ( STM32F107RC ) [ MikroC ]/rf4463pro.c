...
const uint8_t RF4463_RF_GLOBAL_XO_TUNE_2_DATA[] = { RF_GLOBAL_XO_TUNE_2 };





const uint8_t RF4463_RF_GPIO_PIN_CFG_DATA[]     = { RF_GPIO_PIN_CFG };
const unsigned char RF_FRR_CTL_A_MODE_4_data[]  = { RF_FRR_CTL_A_MODE_4}; // NEW
const unsigned char RF_SYNC_CONFIG_6_data   []  = { RF_SYNC_CONFIG_6 }; // NEW
const unsigned char RF_MODEM_RAW_SEARCH2_2_data [] = { RF_MODEM_RAW_SEARCH2_2 };

//0x11, 0x20, 0x02, 0x50, 0x84, 0x0A
...
void vRf4463Init( uint8_t ucSetTxChannel, uint8_t ucSetRxChannel, uint16_t usNetwork, uint8_t ucTxPower, uint8_t ucFrequency ){
    ...
    *********************************************************************/
        ...
        ucRf4463SetCommand( 6, RF4463_CMD_GPIO_PIN_CFG, pcBuffer );  // CAMBIE PROPIEDADES POR COMANDO
        vBufferSetToZero( pcBuffer, 20 );
        //UART1_Write_Text(ucRf4463GetCommand( 6, 0x13, pcBuffer ));
    /*********************************************************************
    ...
    *********************************************************************/
      //  pcBuffer[ 0 ] = 98;
        ...
        pcBuffer[1]  = 0x01;    
        pcBuffer[2]  = 0x00;  
        pcBuffer[3]  = 98;           // freq  adjustment

        ucRf4463SetCommand( RF4463_PROPERTY_GLOBAL_XO_TUNE, 4, pcBuffer );
        vBufferSetToZero( pcBuffer, 20 );
    /*********************************************************************
    ...
    *********************************************************************/
       //pcBuffer[ 0 ] = 0x00;
        ...
        pcBuffer[2]  = 0x01; 
        pcBuffer[3]  = 0x03; 
        pcBuffer[4]  = 0x40;         // tx = rx = 64 byte,PH,high performance mode

        ucRf4463SetCommand( RF4463_PROPERTY_GLOBAL_CONFIG, 5, pcBuffer );  // CAMBIE PROPIEDADES POR COMANDO
        vBufferSetToZero( pcBuffer, 20 );  

    /*********************************************************************
     * Disable all fast response register NEW  CUANDO AGREGO ESTA LINEA PIERDO 10DB EN LA RECEPCION DE SEÑAL
    *********************************************************************/

        ucRf4463SetProperty( RF4463_PROPERTY_FRR_CTL_A_MODE, 8, pcBuffer );  
        vBufferSetToZero( pcBuffer, 20 );      // antes 20 dec
    ...
    *********************************************************************/
        ...
        pcBuffer[2]  = 0x09;    
        pcBuffer[3]  = 0x00;   
        pcBuffer[4]  = 0x08;                         //  8 bytes Preamble
        pcBuffer[5]  = 0x14;                         //  detect 20 bits
        pcBuffer[6]  = 0x00;                     
        pcBuffer[7]  = 0x0f;
        pcBuffer[8]  = 0x31;                         //  no manchest.1010.¡£¡£
        pcBuffer[9]  = 0x00;
        pcBuffer[10]  = 0x00;
        pcBuffer[11]  = 0x00;
        pcBuffer[12]  = 0x00;

       // ucRf4463SetCommand( RF4463_PROPERTY_GLOBAL_CONFIG, 13, pcBuffer ); 
        ucRf4463SetCommand( RF4463_PROPERTY_PREAMBLE_TX_LENGTH, 13, pcBuffer ); // CAMBIE PROPIEDADES POR COMANDO
    /*********************************************************************
     * RF4463 Sync Bytes
    *********************************************************************/
        ...
        pcBuffer[2]  = 0x05;
        pcBuffer[3]  = 0x00;
        pcBuffer[4]  = 0x01;                         // no manchest , 2 bytes
        pcBuffer[5]  = ( usNetwork >> 8 );           // sync byte3
        pcBuffer[6]  = ( usNetwork );                // sync byte2
        pcBuffer[7]  = 0x00;                         // sync byte1
        pcBuffer[8]  = 0x00;                         // sync byte0
    ...
    *********************************************************************/
        ...
        pcBuffer[ 2 ] = 0x00;
        pcBuffer[ 3 ] = 0x20;
        pcBuffer[ 4 ] = 0x81; // 0X05    CHECAR ANTES ESTE VALOR
        ...
        // pcBuffer[2]  = 0x01; 
        // pcBuffer[3]  = 0x00;
        // pcBuffer[4]  = 0x81;                         // CRC = itu-c, enable crc
        // ucRf4463SetProperty( RF4463_PROPERTY_PKT_CRC_CONFIG, 5, pcBuffer );
    /*********************************************************************
...
// Now generate the RF frequency properties
    ...
    xtal_frequency = RADIO_CONFIGURATION_DATA_RADIO_XO_FREQ;
    ...
    //ucRf4463SetProperty( RF_FREQ_CONTROL_INTE_8, 8, freq_control);
...
/*uint8_t pcBuffer[] = { RF_MODEM_RAW_SEARCH2_2_data }; //0x11, 0x20, 0x02, 0x50, 0x84, 0x0A
    ucRf4463SetProperty( RF4463_PROPERTY_INT_CTL_ENABLE, 6, pcBuffer );*/
    //uint8_t pcBuffer[7];

    // pcBuffer[0] = RF4463_CMD_START_TX;
    // pcBuffer[1] = ucTxChannel;
    // pcBuffer[2] = 0x03;
    // pcBuffer[3] = 0x00;
    // pcBuffer[4] = 0x00;
    // pcBuffer[5] = 0x00;
    ...
    ucRf4463SetCommand( 7, RF4463_CMD_START_TX, pcBuffer );
...
//uint8_t pcBuffer[] = { 0x08, 0x7F, 0x00, 0x5D };  // 0X3D
uint8_t pcBuffer[] = { 0x08, 0x7F, 0x00, 0x5D };  // 0X3D
...
void vRf4463EnterRxMode(){
    // uint8_t pcBuffer[7] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x08 };
    uint8_t pcBuffer[7] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }; 
    pcBuffer[ 0 ] = ucRxChannel;


    // pcBuffer[0] = RF4463_CMD_START_RX;
    // pcBuffer[1] = ucRxChannel;
    // pcBuffer[2] = 0x00;
    // pcBuffer[3] = 0x00;
    // pcBuffer[4] = 0x00;
    // pcBuffer[5] = 0x00;
