...
// es mediante USART1 a 115200 8N1. En el interprete de comandos se detallan mas los rangos de operacion de 
// cada parametro.
...
//
// v0.3
//      Envio de mensajes seriales mediante DMA.
//      Borrado y Copiado de buffers por Hardware
//
//      * Escrito sin acentos intencionalmente
//-----------------------------------------------------------------------------------------------------------
void vRF4463MainCommandProcessor ( unsigned char *ucBuffer );

// COMANDOS para procesador de comandos:
// SINTAXIS: Comando|Argumento<CR><LF>
// EJEMPLO1: Set_Power|57<CR><LF>     // Establece la potencia de transmision a 57 decimal
// EJEMPLO3: SET_MATCH_2|120<CR><LF>  // Establece el byte 2 de Match en 0x78 ( modifica la transmision y la recepcion )
// EJEMPLO2: Get_STATUS|<CR><LF>      // Despliega informacion actual de radio
...
unsigned char CMD_RADIO_GET_STATUS         [] = { "GET_STATUS" };    // Probado verificado
unsigned char CMD_RADIO_PACKET_SEND        [] = { "SEND_PACKET" };   // Probado verificado
...
sfr sbit ledEnable   at GPIOA_ODR.B4;         // Salida Enable Led´s
// ARCHIVOS INCLUIDOS CON OPERACIONES
...
#define STM32G071KB       4   // Circuitos de botonera ( SENSORES HOTELES )
...
// Interrupcion para transmision ciclica en modo TX_MASTER
...
// Funciona para colocar los bytes de match ( encabezado )
...
// realiza acciones con los argumentos recibidos. No importa mayusculas o minusculas.
// Los comandos incluyen los rangos aceptables de operacion.
void vRF4463MainCommandProcessor ( unsigned char *ucBuffer ) {
     ...
     // SEND_PACKET
        ...
        if ( ucRF4463Mode == rf_off ) {
           ...
           ucTempChecksum = ucRF4463ChecksumGenerator( ucLongArgument ); //, ucCounterLongArgument );
           
           vUSARTStartMEMtoMEM( ucLongArgument, ucRF4463TXdata + 4, ucCounterLongArgument );      //  ucCounterLongArgument );

           ucRF4463TXdata [ payload_length - 1 ] = ucTempChecksum;

           ucRF4463SendMessage = 1;          // pone la bandera para transmitir
        }
        else {
...
void main() {
     ...
     Delay_ms( 2000 );                    // delay de depuracion
            ...
            ********************************************************/
            ...
            // Basic Init
            // LEDS
            GPIO_Digital_Output( &GPIOB_BASE, _GPIO_PINMASK_6 );  // r   ( Led Rojo de tarjeta de desarrollo )
            GPIO_Digital_Output( &GPIOB_BASE, _GPIO_PINMASK_7 );  // g
            GPIO_Digital_Output( &GPIOB_BASE, _GPIO_PINMASK_8 );  // b
            GPIO_Digital_Output( &GPIOA_BASE, _GPIO_PINMASK_4 );  // Enable Led´s
...
//            EXTI_FTSR.B10   |= 1;
    ...
    // ledEnable =1;
     ...
     ucRF4463Rate  = dr_4p8;            // BAUDRATE
     
     ucRF4463Mode  = tx_test_mode;   //slave_mode;       // para transmision continua usar: tx_test_mode
     
     ucRF4463FreqChannel = 1;          // Establece canal
     ...
     while ( 1 ) {
           ...
           if ( reset_mode == 0 ) {
              ...
              while ( 1 ) {
                    ...
                    // Revisa por temporizador de transmision ciclica
                    if ( Flag.reach_1s ) {
                       Flag.reach_1s = 0;
                       if ( ucRF4463Mode == master_mode ) {
                          //vRF4463TxData();
...
//                                 chksum += rx_buf[ i ];
                             ...
                             //if ( ( chksum == rx_buf[ payload_length - 1 ] )&&( rx_buf[ 4 ] == 0x41 )) {     // FUNCIONA BIEN CON DEMO ABECEDARIO
                             
                             //if ( ( chksum == rx_buf[ payload_length - 1 ] ) ) {
