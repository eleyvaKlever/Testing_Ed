...
void vRF4463Init ( void ) {  // MODE,
     ...
     // Field 1
     app_command_buf[ 4 ]  = 0x00;   // 0x0D  // Field 1 length (?4?)                             // 0x0D
     app_command_buf[ 5 ]  = payload_length;  // field 1 length, (? 8?), ?Total14???        // 0x0E
     app_command_buf[ 6 ]  = 0x04;   // 0x0F  // field 1 Is not 4FSK,manchest, whiting, PN9,       // 0x0F
     app_command_buf[ 7 ]  = 0xaa;   // 0x10  // field 1 crc enble, check enbale, There are also launchingCRC,cRC?Seed ?CRC_seed?????
     
     // Field 2
     app_command_buf[ 8 ]  = 0x00;   // 0x11  // field 2 length(?4?)
     app_command_buf[ 9 ]  = 0x00;   // 0x12  // field 2 length, (? 8?), length = 0 Means thisfield did not use
     app_command_buf[ 10 ] = 0x00;   // 0x13  // field 2 Is not 4FSK,manchest, whiting, PN9
     app_command_buf[ 11 ] = 0x00;   // 0x14  // field 2 ?CRCSet up
     
     // Field 3
     app_command_buf[ 12 ] = 0x00;   // 0x15  // field 3 length, (? 8?), length = 0 Means thisfield did not use
     app_command_buf[ 13 ] = 0x00;   // 0x16  // field 3 length, (? 8?), length = 0 Means thisfield did not use
     app_command_buf[ 14 ] = 0x00;   // 0x17  // field 3 Is not 4FSK,manchest, whiting, PN9
     app_command_buf[ 15 ] = 0x00;   // 0x18  // field 3 ?CRCSet up
...
//        app_command_buf[ 2 ]  = 0x0c;    // Total 12 Parameters
//        app_command_buf[ 3 ]  = 0x00;    // 0x00 Index
//        app_command_buf[ 4 ]  = 's';     // match 1 Value of
...
//        app_command_buf[ 7 ]  = 'w';     // match 2 Value of
//        app_command_buf[ 8 ]  = 0xFF;    // match 2 mask
//        app_command_buf[ 9 ]  = 0x01;    // enable match 2, match 2 The distance between the value of and the synchronization word  , and function
//        app_command_buf[ 10 ] = 'w';     // match 3 Value of
//        app_command_buf[ 11 ] = 0xFF;;   // match 3 mask
//        app_command_buf[ 12 ] = 0x02;;   // enable match 3, match 3 The distance between the value of and the synchronization word    and function
//        app_command_buf[ 13 ] = 'x';     // match 4 Value of
...
//     app_command_buf[ 2 ]  = 0x08;    // Total 8 Parameters
//     app_command_buf[ 3 ]  = 0x00;   // 0x4000
//     app_command_buf[ 4 ]  = 0x3C;   //ucINTE;   //  default value
//     app_command_buf[ 5 ]  = 0x08;   //ucFRAC_2;   //  defaul value
//     app_command_buf[ 6 ]  = 0x00;   //ucFRAC_1;   //  default value
//     app_command_buf[ 7 ]  = 0x00;   //ucFRAC_0;   // frac ,from WDS
//     app_command_buf[ 8 ]  = 0x22;  //step_500K_step1;   // channel step1  from wds  // al parecer aquí esta el ancho del canal
//     app_command_buf[ 9 ]  = 0x22; //step_500K_step0;   // channel step0  from wds
//     app_command_buf[ 10 ] = 0x20;  //  from wds Without this Values
//     app_command_buf[ 11 ] = 0xFF; //ucVCO;    // from wds Without this Values
     spi_write( 12, app_command_buf );
}
...
// Funcionpara transferir datos vía SPI por SOFTWARE
unsigned char ucRf4463SPIByte( unsigned char ucData ) {
ucSPI1ByteReceived = 0;

SPI1_DR = ucData;

while ( !SPI1_SRbits.TXE );                // espera a que el buffer este vacio

while ( SPI1_SRbits.BSY );                // espera a que el buffer este vacio
