...
void vRF4463Init ( void ) {  // MODE,
     ...
     app_command_buf[ 4 ]  = 's';     // 0x00;   //  match 1 Value of
...
void vRF4463TxCont( void ) {
     ...
     LED_RED = 1;
     fifo_reset();  // Buffer empty
     spi_write_fifo();  // Fill the buffer with data
     enable_tx_interrupt();
     clr_interrupt();  // Clear interrupt factor
     tx_start();    // Enter launch mode, start launch
     rf_timeout = 0;
     Flag.rf_reach_timeout = 0;
...
void rf_init_freq ( void ) {
     ...
     spi_write( 5, app_command_buf );
     // 0x11, 0x40, 0x08, 0x00, 0x3B, 0x09, 0x55, 0x55, 0x88, 0x89, 0x20, 0xFF
     app_command_buf[ 0 ]  = 0x11;
     app_command_buf[ 1 ]  = 0x40;    // 0x4000         FREQ_CONTROL_INTE
     app_command_buf[ 2 ]  = 0x08;    // Total 8 Parameters
     app_command_buf[ 3 ]  = 0x00;   // 0x4000
     app_command_buf[ 4 ]  = 0x3B;  //ucINTE;   //  default value
     app_command_buf[ 5 ]  = 0x09;  //ucFRAC_2;   //  defaul value
     app_command_buf[ 6 ]  = 0x55;  //ucFRAC_1;   //  default value
     app_command_buf[ 7 ]  = 0x55;  //ucFRAC_0;   // frac ,from WDS
     app_command_buf[ 8 ]  = step_500K_step1;   // channel step1  from wds  // al parecer aquí esta el ancho del canal
...
unsigned char ucRf4463SPIByte( unsigned char ucData ) {
        ...
        //for ( i = 0; i < 8; i++) {               // Control SCK and SDI, transmit a byte of Command, and read 1 byte of data at the same time  // Does not include nSEL control
//                if ( ucData & 0x80 )
//                        SDI = 1;
//                else
//                        SDI = 0;
...
void VRF4463SDNReset( void ) {
     ...
     SDN = 1;
     Delay_ms( 2 );        // delay_1ms( 2 );        // RF Module reset  DELAY
     SDN = 0;
